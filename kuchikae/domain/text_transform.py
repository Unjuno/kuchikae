    ...
    def transform(self, text: str, prompt: TextTransformPrompt) -> str:
        ...
        try:
            ...
            system_parts = [
                ...
                "禁止:\n",
                "- 入力にない挨拶（「はい」「こんにちは」「おはよう」等）、自己紹介、呼びかけ、前置きを追加しない。\n",
                "- 入力の内容、事実、数値、固有名詞、日時、否定条件を変えない。話し方・文体だけを変える。\n",
                "- 複数案、箇条書き、Markdown、引用符を出さない。\n",
                "- 翻訳しない。要約しすぎない。\n",
                "- 入力と矛盾する内容を足さない。\n",
            ...
            if not few_shot_block:
                system_parts.extend([
                    "出力例:\n",
                    "- 入力: それでね、明日の三時ね、渋谷で待ち合わせなんだ / スタイル: 実況者っぽく / 出力: というわけで明日の15時、渋谷で待ち合わせという体です！\n",
                    "- 入力: それでね、明日の三時ね、渋谷で待ち合わせなんだ / スタイル: 丁寧に / 出力: それで明日の3時に渋谷で待ち合わせなんです。\n",
                    "- 入力: ごめん、遅れるわ / スタイル: 執事っぽく / 出力: 申し訳ございません。少々遅れてしまいます。\n",
                    "- 入力: ごめん、遅れるわ / スタイル: 親しみやすく / 出力: ごめん、ちょっと遅れちゃう！\n",
                    "- 入力: わかった、行くよ / スタイル: 魔王っぽく / 出力: 良かろう、参るとするか。\n",
                    "- 入力: わかった、行くよ / スタイル: ギャルっぽく / 出力: りょー！行くっしょ！\n",
                    "- 入力: ありがとう / スタイル: 自然に / 出力: ありがとうございます。\n",
                    "- 入力: ありがとう / スタイル: 深夜ラジオっぽく / 出力: ありがとうございます。あなたの優しさが沁みます。\n",
                ])
